...
function activeModel(block){
  ...
  const camera = new THREE.PerspectiveCamera(
    75,
    block.offsetWidth / window.innerHeight,
    0.1,
    1000
  );
  ...
  renderer.setSize(block.offsetWidth, block.offsetWidth);
  ...
  loader.load("./glb/chair.glb", function (gltf) {
    model = gltf.scene;
    model.traverse((node) => {
      if (node.isMesh) {
        if (node.material) {
          node.material.metalness = 1.1;
...
let block_info = document.querySelector('.info__3d')
activeModel(block_info)